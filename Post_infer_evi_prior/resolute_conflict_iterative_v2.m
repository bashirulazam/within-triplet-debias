function [resoluted_inferred_triplets,inferred_label_list] = resolute_conflict_iterative_v2(inferred_triplets,measured_triplets,measured_relations_list,predicate_logits_list,obj_logits_list,measured_label_list,test_start,test_ind,top,method,dataset)
%resoluting subject conflict 
...
for i = test_start:test_ind
    ...
    %         labels_in_triplet = [triplets(triplets_ind_as_subject,1); triplets(triplets_ind_as_object,3)];
    %         
    %         mode_label = mode(labels_in_triplet);
    %         triplets(triplets_ind_as_subject,1) = mode_label;
    %         triplets(triplets_ind_as_object,3) = mode_label;
        end
        ...
        %mod_obj_logits = modify_obj_logits(ent_labels);
        %triplets = infer_triplets_image(triplets,relations,mod_obj_logits,predicate_logits,top,pr_r_so,pr_r);
